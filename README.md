...
* Sample on-line plotting while training(avg loss)/testing(write/read weights & memory) DNC on the repeat-copy task (the write/read weights here are after location focus so are no longer necessarily normalized within each head by design):
<img src="/assets/dnc_repeat_copy_train_revised.png" width="205"/> <img src="/assets/dnc_repeat_copy_test_revised.gif" width="600"/>
